#!/usr/bin/env python3
"""Evaluate end-to-end latency for submit-to-all-alive-followers-commit path.

Metric:
  t_all_alive_followers_know_commit - t_client_submit_to_node

Where "all alive followers know commit" means every follower process that is
still alive reaches commit_index >= slot_of_command.
"""
...
def run_experiment(
    ...
    try:
        ...
        for i in range(commands):
            ...
            alive_followers_at_submit = [f for f in followers if nodes[f].proc.poll() is None]
            alive_followers_required = len(alive_followers_at_submit)
                    ...
                    records.append(
                        {
                            ...
                            "alive_followers_required": alive_followers_required,
                            "submit_ts": "",
                            "leader_commit_send_ts": "",
                            "slot": "",
                            "all_alive_follower_commit_ts": "",
                            "latency_ms": "",
            ...
            while time.perf_counter() < deadline and not done:
                ...
                for f in followers:
                    if nodes[f].proc.poll() is not None:
                        continue
                ...
                if slot is not None:
                    alive_followers = [f for f in followers if nodes[f].proc.poll() is None]
                    ...
                    if alive_followers and all(f in follower_commit_ts for f in alive_followers):
                        all_alive_commit_ts = max(follower_commit_ts[f] for f in alive_followers)
                        alive_followers_required = len(alive_followers)
                        latency_ms = (all_alive_commit_ts - t_submit) * 1000.0
                        records.append(
                            {
                                "command": cmd,
                                "cluster_size": cluster_size,
                                ...
                                "slot": slot,
                                "all_alive_follower_commit_ts": f"{all_alive_commit_ts:.9f}",
...
def write_summary(summary_path: Path, source: str, rows: List[Dict[str, object]]) -> None:
    ...
    with summary_path.open("w", encoding="utf-8") as f:
        f.write("Follower-Forward End-to-End Latency (All Alive Followers Commit)\n")
        f.write("===============================================================\n\n")
        f.write("Metric: t(all alive followers know commit by status) - t(client submit to node)\n")
        f.write("Client: this eval script writes requests to a node selected by submit_mode.\n")
        f.write("Threshold: all currently alive followers must have commit_index >= slot.\n")
...
def write_svg(fig_path: Path, rows: List[Dict[str, object]]) -> None:
    ...
    if sweep_axis == "cluster_size":
        title = "End-to-End Latency: Client Submit to All Alive Followers Commit (vs Cluster Size)"
        ...
        title = "End-to-End Latency: Client Submit to All Alive Followers Commit (vs Message Length)"
...
def main() -> int:
    p = argparse.ArgumentParser(description="Submit-to-all-alive-followers-commit evaluator")
    ...
    p.add_argument(
        "--input-csv",
        default="",
        help="CSV path for --mode plot (default: <out-dir>/submit_to_all_alive_followers_commit_samples_*.csv)",
    )
    p.add_argument("--out-dir", default="eval/eval_submit_to_all_alive_followers_commit")
    args = p.parse_args()
    ...
    if args.mode == "plot" and args.input_csv:
        ...
        prefix = "submit_to_all_alive_followers_commit_samples_"
        tag = stem[len(prefix):] if stem.startswith(prefix) else "from_csv"

    csv_path = out_dir / f"submit_to_all_alive_followers_commit_samples_{tag}.csv"
    summary_path = out_dir / f"submit_to_all_alive_followers_commit_summary_{tag}.txt"
    fig_path = out_dir / f"submit_to_all_alive_followers_commit_figure_{tag}.svg"
